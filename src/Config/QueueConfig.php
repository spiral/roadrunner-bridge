...
final class QueueConfig extends InjectableConfig
{
    ...
    public function getDefaultDriver(): string
    {
        if (! isset($this->config['default']) || empty($this->config['default'])) {
            throw new InvalidArgumentException('Default queue pipeline is not defined.');
        }

        if (! \is_string($this->config['default'])) {
            throw new InvalidArgumentException('Default queue pipeline config value must be a string');
        }
     ...
     * @throws InvalidArgumentException
     */
    public function getPipelines(?string $driver = null): array
    {
        $connections = $this->config['pipelines'] ?? $this->config['connections'] ?? [];
    ...
    public function getPipeline(string $name): array
    {
        ...
        if (! isset($pipelines[$name])) {
            throw new InvalidArgumentException(sprintf('Queue pipeline with given name `%s` is not defined.', $name));
        }

        if (! isset($pipelines[$name]['driver'])) {
            throw new InvalidArgumentException(sprintf('Driver for queue pipeline `%s` is not defined.', $name));
        }

        $connection = $pipelines[$name];
        $driver = $connection['driver'];

        if (! \is_string($driver)) {
            throw new InvalidArgumentException(
                sprintf('Driver for queue pipeline `%s` value must be a string', $name)
            );
        }
        ...
        if (! \is_string($connection['driver'])) {
            throw new InvalidArgumentException(
                sprintf('Driver alias for queue pipeline `%s` value must be a string', $name)
            );
        }
